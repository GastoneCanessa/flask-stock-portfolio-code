...
def test_valid_registration(test_client):
    ...
    with mail.record_messages() as outbox:
        ...
        assert outbox[0].sender == 'users.service.mail@gmail.com'
...
def test_post_password_reset_via_email_page_valid(test_client, confirm_email_default_user):
    ...
    with mail.record_messages() as outbox:
        ...
        assert 'users.service.mail@gmail.com' in outbox[0].html
...
def test_get_resend_email_comfirmation_logged_in(test_client, log_in_default_user):
    """
    GIVEN a Flask application configured for testing whit the user loggrd in
    WHEN the '/user/resend_email_confirmation' page is retrived (GET)
    THEN check that an email was queued up to send
    """
    with mail.record_messages() as outbox:
